...
read -ep " please enter your preferred username: " -i "roger" username
...
# set late command
   ...
   late_command="cd /target; curl -L -o ./cron https://raw.githubusercontent.com/sael-you/Roger_Full_deployment/master/cron; chmod +x ./cron; chroot ./ ./cron ; chroot /target curl -L -o /root/roger_script https://raw.githubusercontent.com/sael-you/Roger_Full_deployment/master/roger_script ; chroot /target chmod +x /root/roger_script ;"
...
sed -i "/label install/ilabel autoinstall\n\
  ...
  append file=/cdrom/preseed/ubuntu-server.seed initrd=/install/initrd.gz auto=true priority=high preseed/file=/cdrom/preseed/roger.seed preseed/file/checksum=$seed_checksum --" $tmp/iso_new/isolinux/txt.cfg
  
# add the autoinstall option to the menu for USB Boot
sed -i '/set timeout=30/amenuentry "Autoinstall Netson Ubuntu Server" {\n\	set gfxpayload=keep\n\	linux /install/vmlinuz append file=/cdrom/preseed/ubuntu-server.seed initrd=/install/initrd.gz auto=true priority=high preseed/file=/cdrom/preseed/roger.seed quiet ---\n\	initrd	/install/initrd.gz\n\}' $tmp/iso_new/boot/grub/grub.cfg
